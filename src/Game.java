...
public class Game extends JPanel implements KeyListener{
	...
	private static int speed = 50,MarioSize = 60 ,waveHeight = 70;
	private static int base=400,xStart = 1000;
	private long point = 0,lastPress=0;
	
	private Mario mario = new Mario(100,base-50);
	...
	public void paint(Graphics g) {
			try {
				...
				g2.drawString("Point : "+point,855,40);
				//----Mario----
				g2.setColor(Color.RED);
				drawMarioHealth(g2);
				g2.drawImage(mario.getImage(),mario.x,mario.y,MarioSize,MarioSize, null);
				//----Wave----
	...
	private void drawBackground(Graphics2D g2) throws IOException {
			g2.drawImage(ImageIO.read(new File("src/img/sky.jpg")),0,0,2000,1000, null);
			g2.drawImage(building.getImage(),building.x,building.y,500,200,null);
			g2.drawImage(ImageIO.read(new File("src/img/background.png")),0,base+10,2000,220, null);
			for(Environment item:envSet) {
				g2.drawImage(item.getImage(),item.x,item.y,250,180, null);
			}
	}
	
	private void drawMarioHealth(Graphics2D g2) {
		try {
			g2.drawImage(ImageIO.read(new File("src/img/heart.png")),10,17, 25,25,null);
			g2.setStroke(new BasicStroke(18.0f));
			g2.setColor(new Color(241, 98, 69));
			g2.drawLine(55, 30,55+mario.health,30);	
			g2.setColor(Color.white);
			g2.setStroke(new BasicStroke(2.0f));
			g2.drawRect(45,20, 200,20);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private void drawWave(Wave wave,Graphics2D g2) {
			...
			if(Event.checkHit(mario, wave, MarioSize, waveHeight)){
					g2.setColor(new Color(241, 98, 69));
					g2.fillRect(0, 0,1000,1000);			
					mario.health-=20;
					if(mario.health<=0) {
						display.endGame(this.point);
						mario.health = new Mario().health;
						this.point = 0;	
					}
			}
	}
